...
import {
  ...
  Form,
  Input,
  Modal, Button, DatePicker, Checkbox,
} from 'antd';
...
class UpdateSelf extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.data.record !== this.props.data.record) {
      ...
      const workOrderId = initData.workOrderId;//工作令
      const workOrderName = initData.workOrderName;
      const materialId = initData.materialId;//产品
      const materialName = initData.materialName;
      const watchId = initData.watchId;//制表人
      const watchName = initData.watchName;
      const reviewId = initData.reviewId;//审核人
      const reviewName = initData.reviewName;
      this.setState({
        initData: nextProps.data.record,
        selectedWorkorderRowKeys: [workOrderId],
        SelectWorkorderValue: workOrderName,
        selectedMaterialRowKeys: [materialId],
        SelectMaterialValue: materialName,
        selectedOperationRowKeys: [watchId],
        SelectOperationValue: watchName,
        selectedOperationApRowKeys: [reviewId],
        SelectOperationApValue: reviewName,
      });
    }
  }
  ...
  handleOk = (onSave) => {
    ...
    const { initData, BStatus, selectedWorkorderRowKeys, selectedOperationRowKeys, selectedOperationApRowKeys, selectedMaterialRowKeys } = this.state;
    if (BStatus) {
      return;
    }
      ...
      >
        <div style={{ padding: '0 24px', height: document.body.clientHeight / 1.5, overflow: 'auto' }}>
          <Row gutter={16}>
            <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label="编号">
                {getFieldDecorator('code', {
                  ...
                  initialValue: initData.code ? initData.code : null,
            ...
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label="工作令">
                {getFieldDecorator('workOrderId', {
                  initialValue: this.state.SelectWorkorderValue,
                  rules: [{
                    required: true,
                    message: '工作令必填',
                  }],
            ...
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label="订货单位">
                {getFieldDecorator('orderUnit', {
                  initialValue: initData.orderUnit ? initData.orderUnit : null,
            ...
            <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label="所属行业">
                {getFieldDecorator('industry', {
                  initialValue: initData.industry ? initData.industry : null,
            ...
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label="订货数量">
                {getFieldDecorator('num', {
                  initialValue: initData.num ? initData.num : 0,
                })(<Input placeholder={'请输入订货数量'} type={'number'} />)}
              </Form.Item>
            </Col>
          </Row>
          <Row gutter={16}>
            <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label="技术状态">
                {getFieldDecorator('tech', {
                  initialValue: initData.tech ? initData.tech : 0,
                })(<Input placeholder={'请输入技术状态'} />)}
              </Form.Item>
            </Col>
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label="交付时间">
                {getFieldDecorator('dueTime', {
                  initialValue: initData.tech ? moment(initData.tech) : null,
                })(<DatePicker style={{ width: '100%' }} />)}
              </Form.Item>
            </Col>
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label="军种">
                {getFieldDecorator('militaryType', {
                  initialValue: initData.militaryType ? initData.militaryType : null,
                })(<Input placeholder={'请输入军种'} />)}
              </Form.Item>
            </Col>
          </Row>
          <Row gutter={16}>
            ...
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label="审核人">
                {getFieldDecorator('reviewId', {
                  initialValue: this.state.SelectOperationApValue,
                })(<SelectModelTable
                  on={onpersonAp}
                  data={datapersonAp}
                />)}
          ...
          <Row gutter={16}>
            <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label="制表组">
                {getFieldDecorator('watchGroup', {
                  initialValue: initData.watchGroup ? initData.watchGroup : null,
                })(<Input placeholder={'请输入制表组'} />)}
              </Form.Item>
            </Col>
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label="单据日期">
                {getFieldDecorator('receiptTime', {
                  initialValue: initData.receiptTime ? moment(initData.receiptTime) : null,
                })(<DatePicker style={{ width: '100%' }} />)}
              </Form.Item>
            </Col>
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label="是否准批">
                {getFieldDecorator('isApproval', {
                  initialValue: initData.isApproval ? initData.isApproval : 0,
                  valuePropName: 'checked',
                })(<Checkbox />)}
              </Form.Item>
