...
class UpdateChild extends PureComponent {
  ...
  componentWillReceiveProps(nextProps){
    if(nextProps.data.record !== this.props.data.record){
      ...
      const stationId = initData.stationId;
      const stationName = initData.stationName;
      const batchId = initData.batchId;
      const batchName = initData.batchName;
      this.setState({
        initData:nextProps.data.record,
        SelectMaterialValue:materialName,
        selectedMaterialRowKeys:[materialId],
        SelectAreaValue:stationName,
        selectedAreaRowKeys:[stationId],

        TreeRightKey:[initData.cargoId+""],
        SelectValue:[initData.cargoName],

        SelectBatchValue:batchName,
        selectedBatchRowKeys:[batchId],
  ...
  onSave = (onSave)=>{
    ...
    const { BStatus,selectedMaterialRowKeys,selectedAreaRowKeys,selectedBatchRowKeys,TreeRightKey,initData } = this.state;
    if(BStatus){
      return
    }
          ...
          <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
            <Form.Item label="批次">
              {getFieldDecorator('batchId',{
                initialValue:this.state.SelectBatchValue,
              })( <ModelTable
                on={onBatchOn}
                data={onBatchData}
              />)}
          ...
          <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
            <Form.Item label="单价">
              {getFieldDecorator('unit',{
                initialValue:initData.unit?initData.unit:'',
              })(
          ...
          <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
            <Form.Item label="金额">
              {getFieldDecorator('mny',{
                initialValue:initData.mny?initData.mny:'',
              })(
