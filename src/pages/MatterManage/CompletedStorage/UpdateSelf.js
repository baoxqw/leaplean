...
class UpdateChild extends PureComponent {
  ...
  componentWillReceiveProps(nextProps){
    if(nextProps.data.record !== this.props.data.record){
      ...
      const planId = initData.psnId;
      const planName = initData.psnName;
  ...
  onSave = (onSave)=>{
    ...
    const { BStatus,selectedOperationRowKeys,initData,selectedProductRowKeys } = this.state;
    if(BStatus){
      return
    }
          ...
          <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
            <Form.Item label="单据状态">
              {getFieldDecorator('status',{
                initialValue:initData?initData.status:0,
              })(
