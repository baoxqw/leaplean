...
class index extends PureComponent {
  ...
  filterData = ({value="",treeId=null,pageIndex = 0,pageSize = 10})=>new Promise(resolve => {
    const { data:{ tableConditions = [],treeCode,tableType },dispatch } = this.props;
    let obj = {
      pageIndex,
      pageSize,
      reqData:{},
    ...
    if (treeType && tableType) {
      ...
      dispatch({
        ...
        callback: async (res1) => {
          ...
          const res2 = await this.filterData({})
          this.setState({
            treeData:tree,
            tableData:res2
          })
  ...
  onSelect = async (selectedKeys, info) => {
    ...
    if (info.selectedNodes[0]) {
      const res = await this.filterData({
        value:queryValue,treeId:
        info.selectedNodes[0].props.dataRef.id
      })
      this.setState({
  ...
  handleTableChange = async (pagination) => {
    const {  queryValue,treeId } = this.state;
    const res = await this.filterData({
      value:queryValue,
      treeId,
  ...
  onSearchSelect = (value)=>{
    ...
    const res = this.filterDataQian(tableData,value,null,0,10000000,true)
    this.setState({
      selectData:res
    })
  }
  ...
  render() {
    ...
    const { data,loading, selectData,treeData,tableData, selectedRowKeys, searchValue, expandedKeys, autoExpandParent, selectedKeys } = this.state;
