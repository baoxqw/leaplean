...
class index extends PureComponent {
  ...
  filterData = (dataList,value="",pageIndex = 0,pageSize = 10,bool=false)=>{
  ...
  onIconClick = () => {
    ...
    if (tableType) {
      ...
      dispatch({
        ...
        payload: {},
        callback: (res) => {
          const selectData = this.filterData(res.list)
          this.setState({
            selectData,
            tableData:res
          });
  ...
  onHandleSearch = (e) => {
    ...
    form.validateFields((err, values) => {
      if (err) return;
      const selectData = this.filterData(tableData.list,values.code)
      this.setState({
  ...
  onHandleReset = () => {
    ...
    const selectData = this.filterData(tableData.list,"",0,page.pageSize)
    this.setState({
      selectData,
      queryValue:'',
      expandedKeys: [],
  ...
  handleTableChange = (pagination) => {
    const {  tableData,queryValue } = this.state;
    const selectData = this.filterData(tableData.list,queryValue,pagination.current - 1,pagination.pageSize)
    this.setState({
      selectData,
      page:{
  ...
  onFocusSelect = () => {
    ...
    dispatch({
      ...
      payload: {},
      callback: (res) => {
        const selectData = this.filterData(res.list,"",0,10,true)
        this.setState({
          selectData,
          tableData:res,
          loading:false
  ...
  onSearchSelect = (value)=>{
    ...
    const selectData = this.filterData(tableData.list,value,0,10,true)
    this.setState({
      selectData
    })
  }
  ...
  render() {
    ...
    const { data,loading,selectData, selectedRowKeys, searchValue } = this.state;
