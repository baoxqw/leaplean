...
const dataAddLabel = (data) => {
  data.map((item) => {
    item.label = item.name;
    if(item.children){
      dataAddLabel(item.children)
    }
  })
}


function toTreeMaterial(data) {
  // 删除 所有 children,以防止多次调用
  data.forEach(function (item) {
    delete item.routes;
  });
  ...
  data.forEach((item)=>{
    item.key = item.id;
    // 以当前遍历项的pid,去map对象中找到索引的id
    let parent = map[item.fatherMaterialId];
    // 好绕啊，如果找到索引，那么说明此项不在顶级当中,那么需要把此项添加到，他对应的父级中
    if (parent) {
      (parent.children || ( parent.children = [] )).push(item);
    } else {
...
function toTreeSpecify(data,id,pid) {
  // 将数据存储为 以 id 为 KEY 的 map 索引数据列
  let map = {};
  data.forEach((item) =>{
    map[item[id]] = item;
