...
@Form.create()
class WorkOrderUpdate extends PureComponent {
  state = {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.data.record !== this.props.data.record) {
      ...
      console.log("record")
      this.setState({
        SelectOperationValue: record.applicantName,
        selectedOperationRowKeys: [record.applicantId],

        SelectHitchValue: record.developmentName,
        selectedHitchRowKeys: [record.developmentId],

        deptId: record.deptId,//申请部门
        deptName: record.deptName,
        modelId: record.pid,
        modelName: record.pname,
        initData: record,
      });
    }
  }
  ...
  handleOk = (onOk) => {
    ...
    const { BStatus,selectedHitchRowKeys, selectedOperationRowKeys, deptId, modelId, initData } = this.state;
    if(BStatus){
      return;
    }
  ...
  render() {
    ...
    const { initData, SelectHitchValue, SelectOperationValue, deptName,modelName } = this.state;
          ...
          <Row gutter={16}>
            <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label='工作令编码'>
                ...
                })(<Input placeholder='请输入工作令编码' disabled/>)}
              </Form.Item>
            </Col>
            ...
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label='研制状态'>
                {form.getFieldDecorator('developmentId', {
          ...
          <Row gutter={16}>
            <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label='项目分类号'>
                {form.getFieldDecorator('developCategory', {
                  initialValue: initData.developCategory ? initData.developCategory : '',
                })(
            ...
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label='工作令性质'>
                {form.getFieldDecorator('developNature', {
                  initialValue: initData.developNature ? initData.developNature : '',
                })(
            ...
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label='工作令批次'>
                {form.getFieldDecorator('developBatch', {
                  initialValue: initData.developBatch ? initData.developBatch : '',
                })(<Input placeholder='请输入工作令批次' />)}
              </Form.Item>
            </Col>
          </Row>
          <Row gutter={16}>
            <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label='项目名称'>
                {form.getFieldDecorator('modelNumber', {
                  initialValue: initData.modelNumber ? initData.modelNumber : '',
            ...
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label='经费来源'>
                {form.getFieldDecorator('sourceOfFunding', {
                  initialValue: initData.sourceOfFunding ? initData.sourceOfFunding : '',
          ...
          <Row gutter={16}>
            <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label='终止时间'>
                ...
                })(<DatePicker style={{ width: '100%' }} placeholder={'请选择终止时间'} />)}
              </Form.Item>
            </Col>
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label='停止标志'>
                {form.getFieldDecorator('stopSign', {
                  initialValue: initData.stopSign ? initData.stopSign : '',
                })(<Input placeholder='请输入停止标志' />)}
              </Form.Item>
            </Col>
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label="申请人">
            ...
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
              <Form.Item label='延期使用日期'>
                {form.getFieldDecorator('extension', {
                  initialValue: initData.extension ? moment(initData.extension) : null,
                })(<DatePicker style={{ width: '100%' }} placeholder={'请选择延期使用日期'} />)}
              </Form.Item>
            </Col>
            <Col lg={{ span: 6, offset: 2 }} md={{ span: 10, offset: 1 }} sm={24}>
