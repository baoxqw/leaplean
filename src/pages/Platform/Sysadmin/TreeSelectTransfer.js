...
class TreeSelectTransfer extends Component {
  ...
  allclick = (e)=>{
    ...
    fLeftData.map((item)=>{
      if(item.children){
        item.children.map((itec)=>{
          a.push(itec.key)
        })
       if(item.children.children){
         item.children.children.map((itechi)=>{
           a.push(itechi.key)
         })
         if(item.children.children.children){
           item.children.children.children.map((itechildren)=>{
             a.push(itechildren.key)
           })
         }
       }
      }
    ...
    return (
      <div className={styles['tree-select-transfer']}>
        <div className="tst-l">
          ...
          <div style={{ overflow: 'auto', height: 220 }}>
            <div style={{marginLeft:'10px'}}>
              <Checkbox onChange={this.allclick} checked={leftAllChecked}>全选</Checkbox>
            </div>
            <Tree
              checkable
        ...
        <div className="tst-l">
          ...
          <div style={{ overflow: 'auto', height: 220 }}>
            <div style={{marginLeft:'10px'}}>
              <Checkbox onChange={this.allclickRight} checked={rightAllChecked} disabled={fRightData.length<1}>全选</Checkbox>
            </div>
            <Tree
              checkable
              defaultExpandAll
