...
@Form.create()
class MaterialAdd extends PureComponent {
  state = {
    SelectValue: [],
    selectedRowKeys: [],
    graphidStatus: false,
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.data.record !== this.props.data.record){
      const { data:{ record } } = nextProps;
      this.setState({
        invclName:record.name,
        invclId:record.id
      })
    }
  }
  ...
  handleOk = (onOk)=>{
    ...
    const { selectedRowKeys,invclId,BStatus,selectedOperationRowKeys } = this.state;
    if(BStatus){
      return;
    }
