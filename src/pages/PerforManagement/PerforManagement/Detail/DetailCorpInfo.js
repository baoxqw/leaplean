...
import { Card, Button,Form, Col, Row, DatePicker, Input, Select, Cascader, Checkbox, Anchor } from 'antd';
...
import FinanceTableForm from './FinanceTableForm';
import ShareholderTableForm from './ShareholderTableForm';
import FinancingTableForm from './FinancingTableForm';
import LicenserouterTableForm from './LicenseTableForm';
import TeamTableForm from './TeamTableForm';
import DistributionPlanTableForm from './DistributionPlanTableForm';
...
@connect(({ postmanagement, loading }) => ({
  postmanagement,
  data:postmanagement.detail,//详细信息
  shareholder:postmanagement.shareholder,//股东结构
  historical:postmanagement.history,//融资历史
  distribution:postmanagement.distribution,//分配方案
  team:postmanagement.team,//团队
  finance:postmanagement.finance,//公司财务
  license:postmanagement.license,//牌照
  investplan:postmanagement.investplan,
  loading: loading.models.postmanagement,
}))

class DetailCorpInfo extends PureComponent {
  state = {
    initData:{},
  ...
  componentDidMount() {
    ...
    let str  = u .substring(index + 1, u.length);
    const obj = {
      reqData:{
        id:str
      }
    ...
    dispatch({
      ...
      callback:(res)=>{
        ...
        const htmlContent = res[0].projectDetail.corpdesc;
        const htmlContentt = res[0].projectDetail.roadmap;
        const htmlContento = res[0].projectDetail.hotcommunity;
  ...
  fiHisColumns = [
    { title: '融资时间', dataIndex: 'fiDate' },
    { title: '融资轮数', dataIndex: 'fiStep' },
    { title: '融资类型', dataIndex: 'fiType' },
    { title: '投资机构', dataIndex: 'investOrg' },
    { title: '投资金额（单位：万元）', dataIndex: 'investAmount' },
    { title: '公司估值（单位：万元）', dataIndex: 'corpValue' }
  ];

  lcsColumns = [
    { title: '拍照名称', dataIndex: 'lcsName' },
    { title: '状态', dataIndex: 'clsState' },
    { title: '预计取得时间', dataIndex: 'planGetDate' }
  ];
  ...
  render() {
    ...
    const {  editorState, editorState1, editorState2 } = this.state;
    if(investplan){
      if(investplan.investtype == 1){
        investplan.investtype = formatMessage({ id: 'validation.equity' })
      }else if(investplan.investtype == 0){
        investplan.investtype = 'Token '
      }

      if(investplan.stocktype == 1){
        investplan.stocktype = formatMessage({ id: 'validation.newshares' })
      }else{
        investplan.stocktype = formatMessage({ id: 'validation.oldstock' })
      }

      if(investplan.investtool == 'common'){
        investplan.investtool = formatMessage({ id: 'validation.commonstock' })
      }else if(investplan.investtool == 'prefer'){
        investplan.investtool = formatMessage({ id: 'validation.preferredstock' })
      }else if(investplan.investtool == 'cb'){
        investplan.investtool = formatMessage({ id: 'validation.debttoequity' })
      }
      ...
      <Row>
        <Col span={22}>
          <Form layout="vertical" loading={loading}>
            <Card title={formatMessage({ id: 'validation.projectinformation' })} className={styles.card} bordered={false} id='1'>
              <Row gutter={16}>
                <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
                  <Form.Item label={formatMessage({ id: 'form.title.projectname' })}>
                    {getFieldDecorator('project_name', {
                      initialValue:data?data.project_name:'',
                      rules: [{ required: true}],
                    })(<Input disabled/>)}
                  </Form.Item>
                </Col>
                <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
                  <Form.Item label={formatMessage({ id: 'validation.investmentsubject' })}>
                    {getFieldDecorator('maininvestor', {
                      initialValue:data?data.maininvestor:'',
                      rules: [{ required: true }],
                    })(<Input disabled/>)}
                  </Form.Item>
                </Col>
                <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
                  <Form.Item label={formatMessage({ id: 'validation.date' })}>
                    {getFieldDecorator('createdate', {
                      initialValue:data?moment(data.createdate):null,
                      rules: [{ required: true }],
              ...
              <Row gutter={16}>
                <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
                  <Form.Item label={formatMessage({ id: 'validation.label' })}>
                    {getFieldDecorator('tag', {
                      initialValue:data?data.tag:'',
                      rules: [{ required: true, message: '请选择标签' }],
                    })(
                      <Select placeholder="请选择标签" disabled>
                ...
                <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
                  <Form.Item label={formatMessage({ id: 'validation.currency' })}>
                ...
                <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
                  <Form.Item label='国家'>
                    {getFieldDecorator('country', {
                      ...
                      <Select placeholder="请选择国家" disabled>
                ...
                <Col  lg={{ span: 24 }} md={{ span: 24 }} sm={24}>
                  <Form.Item label='地区'>
                    {getFieldDecorator('region', {
                      initialValue:data?[data.province,data.city,data.region]:'',
                      rules: [{ required: true, message: '请选择地区/国家' }],
                    })(
                      <Cascader
                        disabled
              ...
              <Row gutter={16}>
                <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
                  ...
                  <Form.Item label=''>
                    {getFieldDecorator('istoken',{
                      ...
                      <Checkbox disabled>是否涉及Token</Checkbox>
                    )}
                  </Form.Item>
                </Col>
                <Col xl={{ span: 16, offset: 2 }} lg={{ span: 18 }} md={{ span: 12 }} sm={24}>
                  <Form.Item label='公司地址'>
                    ...
                    })(<Input placeholder="请输入公司地址" disabled/>)}
                  </Form.Item>
                </Col>
              </Row>
                ...
                <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
                  <Form.Item label='联系人'>
                    {getFieldDecorator('contactperson', {
                      initialValue:data?data.contactperson:'',
                ...
                <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
                  <Form.Item label='邮箱'>
                    {getFieldDecorator('contactemail', {
                      initialValue:data?data.contactemail:'',
                      rules: [
                    ...
                    })(<Input placeholder="请输入联系人邮箱" disabled/>)}
                  </Form.Item>
                </Col>
                <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
                  <Form.Item label='电话（手机）'>
                    {getFieldDecorator('contactmp', {
                      initialValue:data?data.contactmp:'',
                      // rules: [
                      //   {
                      ...
                      //   },
                      // ],
                    })(<Input placeholder="请输入联系人电话" type='number' disabled/>)}
                  </Form.Item>
                </Col>
              </Row>
              <Row gutter={16}>
               <Col lg={{ span: 22, offset: 1 }} md={{ span: 22, offset: 1 }} sm={24}>
                  <Form.Item label='上市代码'>
                    {getFieldDecorator('ticker', {
                      initialValue:data?data.ticker:'',

                    })(<Input placeholder="上市代码" disabled/>)}
                  </Form.Item>
                </Col>
              </Row>
              <Row gutter={16}>
               <Col lg={{ span: 22, offset: 1 }} md={{ span: 22, offset: 1 }} sm={24}>
                  <Form.Item label='标的简介'>
                    {getFieldDecorator('subjectdesc',{
                      initialValue:data?data.subjectdesc:'',
                    })(<TextArea disabled/>)}
                  </Form.Item>
                </Col>
              </Row>
            </Card>
            <Card title="公司简介" className={styles.card} bordered={false} id='2'>
                <BraftEditor
                  disabled
            ...
            <Card title="路线图" className={styles.card} bordered={false} id='3'>
                <BraftEditor
                  disabled
            ...
            <Card title="社区热度" className={styles.card} bordered={false} id='4'>
                <BraftEditor
                  disabled
            ...
            <Card title="公司财务" className={styles.card} bordered={false} id='5'>
              {getFieldDecorator('finance', {
                initialValue: finance?finance:[],
              })(<FinanceTableForm/>)}
            </Card>

            <Card title="股东结构"  bordered={false} id='6'>
              {getFieldDecorator('shareholder', {
                initialValue: shareholder?shareholder:[],
              })(<ShareholderTableForm />)}
            </Card>

            <Card title="融资历史"  bordered={false} id='7'>
              {getFieldDecorator('history', {
                initialValue: historical?historical:[],
              })(<FinancingTableForm />)}
            </Card>

       {/*     <Card title="牌照信息" bordered={false} id='8'>
              {getFieldDecorator('license', {
                initialValue: license?license:'',
              })(<LicenserouterTableForm />)}
            </Card>*/}

            <Card title="分配方案" bordered={false} id='9'>
              {getFieldDecorator('distribution', {
                initialValue: distribution?distribution:'',
              })(<DistributionPlanTableForm />)}
            </Card>

            <Card title="团队" bordered={false} id='10'>
              {getFieldDecorator('team', {
                initialValue: team?team:'',
              })(<TeamTableForm />)}
            </Card>
            <Row id='11'>
              <Card title="投资计划（股权）" className={styles.card} bordered={false}>
                <Form
                  layout="vertical"
                    ...
                    <Col xl={{ span: 24 }} lg={{ span: 24 }} md={{ span: 24 }} sm={24}>
                      <Form.Item label='备注'>
                        {getFieldDecorator('memo',{
                          ...
                          <TextArea placeholder="请输入备注" disabled/>
                        )}
                      </Form.Item>
                    </Col>
                  </Row>
        ...
        <Col span={2}>
          <Anchor>
            ...
            <Link href="#3" title="线路图" />
            <Link href="#4" title="公司热度" />
            ...
            <Link href="#11" title="投资计划(股权)" />
            <Link href="#12" title="投资计划(Token)" />
