...
@connect(({ holder,loading }) => ({
  holder,
  loading: loading.models.holder,
  datavale:holder.datasource,
  // submitting: loading.effects['form/submitProjectAddForm'],
}))

@Form.create()
class DetailShareholderInfo extends PureComponent {
  ...
  handleUpload = e => {
    ...
    dispatch({
      type: 'holder/submitProjectAddForm',
      payload: formData,
      callback: () => {
        ...
        <GridContent>
          <Row gutter={24}>
            <Col style={{marginBottom:'40px'}}>
              <Card title={formatMessage({ id: 'validation.shareholderinformation' })} bordered={false}>
                <Form onSubmit={this.handleSubmit}  style={{ marginTop: 8 ,marginLeft:0}}>
...
export default DetailShareholderInfo;
