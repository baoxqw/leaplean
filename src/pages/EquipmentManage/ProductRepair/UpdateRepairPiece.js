...
@Form.create()
class UpdateRepairPiece extends PureComponent {
  state = {
    BStatus:false,
    TreeMaterialData:[],//物料
    MaterialConditions:[],
  ...
  componentWillReceiveProps(nextProps){
    if(nextProps.data.record !== this.props.data.record){
      ...
      const { materialId,materialName } = initData;
      this.setState({
  ...
  onSave = (onSave)=>{
    ...
    form.validateFields((err,values)=>{
      ...
      const obj = {
        reqData:{
          id:initData.id,
          maintenancereportId:initData.maintenancereportId,
...
export default UpdateRepairPiece;
