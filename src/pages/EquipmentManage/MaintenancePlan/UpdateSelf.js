...
class UpdateChild extends PureComponent {
  ...
  componentWillReceiveProps(nextProps){
    if(nextProps.data.record !== this.props.data.record){
      ...
      const { equipmentId,deptId,psndocId } = initData;
      this.setState({
        initData:nextProps.data.record,
        selectedStatusRowKeys:[equipmentId],
        selectedStatusValue:initData.equipmentName,
        departmentId:deptId,
  ...
  onSave = (onSave)=>{
    ...
    const { BStatus,selectedPersonRowKeys,selectedStatusRowKeys,initData  } = this.state;
    if(BStatus){
      return
    }
          ...
          <Col lg={{ span: 6, offset: 1 }} md={{ span: 10, offset: 1 }} sm={24}>
            <Form.Item label="设备卡片">
              {getFieldDecorator('equipmentId',{
                ...
                initialValue:this.state.SelectStatusValue
              })(<ModelTable
                on={onStatus}
                data={onStatusData}
              />)}
