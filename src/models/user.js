...
export default {
  ...
  effects: {
    ...
    *fetchUserTaskMsg({ payload }, { call, put }) {
      ...
      let { resData: data, total, userDefineInt1: unreadCount} = response;
      if(response.resData){
        response.resData.map((item)=>{
          item.datetime = item.senddate;
          item.title = `${item.code} ${item.title} ${item.content?item.content:''}`;
          return item
        })
      }
      if(!data){
  ...
  reducers: {
    ...
    setUserTaskMsg0(state, { payload: userTaskMsg0 }) {
      return { ...state, userTaskMsg0 };
    },
